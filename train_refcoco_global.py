...
from agent.refcocoenv_global import RefcocoEnv
...
class MySimulatorMaster(SimulatorMaster, Callback):
  ...
  def _setup_graph(self):
    ...
    self.async_predictor = MultiThreadAsyncPredictor(
        self.trainer.get_predict_funcs(['state'], ['logitsT', 'pred_value', 'rnn_state'],
        PREDICTOR_THREAD), batch_size=15)
    self.async_predictor.run()

  def _on_state(self, state, ident):
    def cb(outputs):
      distrib, value, rnn_state = outputs.result()

      action = np.random.choice(len(distrib), p=distrib)

      client = self.clients[ident]
      client.memory.append(TransitionExperience(state, action, None, value=value))
      self.send_queue.put([ident, dumps([action, rnn_state,])])
...
#      PeriodicCallback(Evaluator(EVAL_EPISODE, ['state'], ['logits', 'rnn_state']), 1),
      GlobalStepSetter(),
    ]),
