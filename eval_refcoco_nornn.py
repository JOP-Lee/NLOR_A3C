...
from agent.refcocoenv_nornn import RefcocoEnvEval
...
def evaluate(cfg):
  ...
  while buf_size != 0:
    ...
    if buf_size == 0:
      break
    probs = get_predict(state_buf)[0]

    for i in range(buf_size):
      identity = identity_buf[i]
      prob = probs[i]
      action = np.random.choice(len(prob), p=prob)
      s2c_socket.send_multipart([identity, dumps([action, ])], copy=False)
...
class Model(ModelDesc):
  def _get_input_vars(self):
    ...
    return [InputVar(tf.float32, (None, WIN_VECTOR_SIZE + VISUAL_SIZE + LANG_VECTOR_SIZE + HISTORY_LENGTH*NUM_ACTIONS), 'state'),
